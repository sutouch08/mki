...
class Customer_class extends PS_Controller
{
  ...
  {
		$code = get_filter('code', 'c_class_code', '');
		$name = get_filter('name', 'c_class_name', '');
  ...
  public function add_new()
  {
    ...
    $this->title = 'เพิ่ม เกรดลูกค้า';
    $this->load->view('masters/customer_class/customer_class_add_view', $data);
  }
  ...
  public function add()
  {
    if($this->input->post('code'))
    {
      ...
      if($this->customer_class_model->is_exists($code) === TRUE)
      {
        $sc = FALSE;
        set_error("'".$code."' มีในระบบแล้ว");
      }

      if($this->customer_class_model->is_exists_name($name) === TRUE)
      {
        $sc = FALSE;
      ...
      if($sc === TRUE)
      {
        if($this->customer_class_model->add($ds))
        {
  ...
  public function update()
  {
    ...
    if($this->input->post('code'))
    {
      $old_code = $this->input->post('customer_class_code');
      $old_name = $this->input->post('customer_class_name');
      $code = $this->input->post('code');
      $name = $this->input->post('name');
      ...
      if($sc === TRUE && $this->customer_class_model->is_exists($code, $old_code) === TRUE)
      {
        $sc = FALSE;
        set_error("'".$code."' มีอยู่ในระบบแล้ว โปรดใช้รหัสอื่น");
      }

      if($sc === TRUE && $this->customer_class_model->is_exists_name($name, $old_name) === TRUE)
      {
        $sc = FALSE;
      ...
      if($sc === TRUE)
      {
        if($this->customer_class_model->update($old_code, $ds) === TRUE)
        {
    ...
    if($sc === FALSE)
    {
      $code = $this->input->post('customer_class_code');
    }
  ...
  public function delete($code)
  {
    if($code != '')
    {
      if($this->customer_class_model->delete($code))
      {
  ...
  public function clear_filter()
	{
		clear_filter(array('c_class_code', 'c_class_name'));
		echo 'done';
	}

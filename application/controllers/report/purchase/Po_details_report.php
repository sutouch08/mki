...
class Po_details_report extends PS_Controller
{
  public $menu_code = 'RPUPOC';
	public $menu_group_code = 'RE';
  public $menu_sub_group_code = 'REPO';
	public $title = 'รายงาน ตรวจสอบใบสั่งซื้อ แสดงราคาซื้อ';
  public $filter;
  public function __construct()
  {
    parent::__construct();
    $this->home = base_url().'report/purchase/po_details_report';
    $this->load->model('report/purchase/po_details_report_model');
  }

  public function index()
  {
    $this->load->view('report/purchase/report_po_details');
  }
  ...
  public function get_report()
  {
    if(!empty($this->input->get()))
    {
      ...
      $result = $this->po_details_report_model->get_data($arr);
      $ds = array();
      if(!empty($result))
      {
        $total_qty = 0;
        ...
        foreach($result as $rs)
        {
          ...
          $arr = array(
            ...
            'status' => ($rs->status == 2 ? 'part' : ($rs->status == 3 ? 'closed' : 'open'))
          );
  ...
  public function do_export()
  {
    ...
    $title = "รายงาน ตรวจสอบใบสั่งซื้อ วันที่ (".thai_date($from_date, FALSE, '/').") - (".thai_date($to_date, FALSE, '/').") วันที่พิมพ์รายงาน : ".thai_date(date('Y-m-d'));
    $vendor = $all_vendor == 1 ? 'ทั้งหมด' : "{$vendor_from} - {$vendor_to}";
    $po = $all_po == 1 ? 'ทั้งหมด' : "{$po_from} - {$po_to}";
    $status = $po_status == 'C' ? 'ปิดแล้ว' : ($po_status === 'O' ? "เปิด" : "ทั้งหมด" );
    ...
    $this->excel->getActiveSheet()->setTitle('PO Details Report');
    ...
    $this->excel->getActiveSheet()->mergeCells('A5:I5');
		...
		$this->excel->getActiveSheet()->setCellValue('A6', '*** กรณีที่เลือกแสดงผลเป็นรุ่นสินค้า ราคาจะเป็นราคาเฉลี่ยตามรุ่นสินค้าในใบสั่งซื้อ');
    //--- set Table header
    $this->excel->getActiveSheet()->setCellValue('A7', 'ลำดับ');
    $this->excel->getActiveSheet()->setCellValue('B7', 'วันที่');
    $this->excel->getActiveSheet()->setCellValue('C7', 'สินค้า');
    $this->excel->getActiveSheet()->setCellValue('D7', 'ใบสั่งซื้อ');
    $this->excel->getActiveSheet()->setCellValue('E7', 'ผู้ขาย');
    $this->excel->getActiveSheet()->setCellValue('F7', 'กำหนดรับ');
		$this->excel->getActiveSheet()->setCellValue('G7', 'ราคา');
    $this->excel->getActiveSheet()->setCellValue('H7', 'จำนวน');
    $this->excel->getActiveSheet()->setCellValue('I7', 'รับแล้ว');
    $this->excel->getActiveSheet()->setCellValue('J7', 'ค้างร้บ');
    $this->excel->getActiveSheet()->setCellValue('K7', 'สถานะ');
    ...
    if(!empty($result))
    {
      ...
      foreach($result as $rs)
      {
        ...
        $this->excel->getActiveSheet()->setCellValue('I'.$row, $rs->received);
        $this->excel->getActiveSheet()->setCellValue('J'.$row, $backlogs);
        $this->excel->getActiveSheet()->setCellValue('K'.$row, ($rs->status == 2 ? 'part' : ($rs->status == 3 ? 'closed' : 'open')));
        $total_qty += $rs->qty;
        $total_received += $rs->received;
        $total_backlogs += $backlogs;
        $no++;
      ...
      $this->excel->getActiveSheet()->setCellValue('H'.$row, $total_qty);
      $this->excel->getActiveSheet()->setCellValue('I'.$row, $total_received);
      $this->excel->getActiveSheet()->setCellValue('J'.$row, $total_backlogs);

      $this->excel->getActiveSheet()->getStyle('A'.$row)->getAlignment()->setHorizontal('right');
      $this->excel->getActiveSheet()->mergeCells("A{$row}:G{$row}");

      $this->excel->getActiveSheet()->getStyle('G7:K'.$row)->getAlignment()->setHorizontal('center');
      $this->excel->getActiveSheet()->getStyle('G7:J'.$row)->getNumberFormat()->setFormatCode('#,##0');

    }

    setToken($token);

    $file_name = "รายงานตรวจสอบใบสั่งซื้อ.xlsx";

...
class Check_stock_diff_model extends CI_Model
{
  ...
  {
		$this->db
		->select('stock.product_code, products.barcode AS barcode, stock.qty AS OnHandQty')
		->from('stock')
		->join('products', 'stock.product_code = products.code', 'left')
		->where('stock.qty !=', 0, FALSE)
		->where('stock.zone_code', $zone_code);

    if(!empty($product_code))
    {
			$this->db->like('stock.product_code', $product_code);
    }

    $this->db->order_by('stock.product_code', 'ASC');
